...
class hn_captcha
{
    ...
    public $TTF_RANGE = ['COMIC.TTF', 'JACOBITE.TTF', 'LYDIAN.TTF', 'MREARL.TTF', 'RUBBERSTAMP.TTF', 'ZINJARON.TTF'];
    ...
    public function __construct($config, $secure = true)
    {
        ...
        $this->messages = [
                'de' => [
                            'msg1' => 'Du mußt die <b>' . $this->chars . ' Zeichen</b> im Bild, (Zahlen&nbsp;von&nbsp;<b>0&nbsp;-&nbsp;9</b> und Buchstaben&nbsp;von&nbsp;<b>A&nbsp;-&nbsp;F</b>),<br>in das Feld eintragen und das Formular abschicken um den Download zu starten.',
                            'msg2' => 'Ohje, das kann ich nicht lesen. Bitte, generiere mir eine ',
                            'buttontext' => 'abschicken',
                            'refreshbuttontext' => 'neue ID',
                            ],
                'en' => [
                            'msg1' => 'You must type the <b>' . $this->chars . ' characters</b> within <b>0..9</b> and <b>A..F</b>, and submit the form.',
        ...
        $this->QUERY_STRING = mb_strlen(trim($_SERVER['QUERY_STRING'])) > 0 ? '?' . strip_tags($_SERVER['QUERY_STRING']) : '';
        ...
        if (isset($_POST['public_key'])) {
            $this->public_K = mb_substr(strip_tags($_POST['public_key']), 0, $this->chars);
        }

        if (isset($_POST['private_key'])) {
            $this->private_K = mb_substr(strip_tags($_POST['private_key']), 0, $this->chars);
        }
        ...
        $this->public_key = mb_substr(md5(uniqid(mt_rand(), true)), 0, $this->chars);
    ...
    public function display_form()
    {
        ...
        $s .= '<br>To help us identify authentic registrations, please enter the letters & numbers as it\'s shown in the box above: ';
    ...
    /** @private *
     * @param bool $onlyTheImage
     * @return string
     */

    public function display_captcha($onlyTheImage = false)
    {
        $this->make_captcha();
    ...
    public function public_key_input()
    {
    ...
    public function make_captcha()
    {
        ...
        for ($i = 0, $x = (int)mt_rand($this->minsize, $this->maxsize); $i < $this->chars; $i++) {
            $text = mb_strtoupper(mb_substr($private_key, $i, 1));
        ...
        if (!$res) {
            die('Unable to safe captcha-image.');
        }
    }
    ...
    public function check_captcha($public, $private)
    {
        ...
        if (file_exists($this->get_filename($public))) {
            ...
            $res = (mb_strtolower($private) == mb_strtolower($this->generate_private($public))) ? 'TRUE' : 'FALSE';
    ...
    */
    ...
    public function get_filename_url($public = '')
    {
        ...
        return str_replace(XOOPS_ROOT_PATH, '', $this->tempfolder) . $public . '.jpg';
    }
    ...
    public function get_try($in = true)
    {
        ...
        if ($in) {
            return (int)mb_substr(strip_tags($_POST['hncaptcha']), ($this->secretposition - 1), 1);
        }  
    ...
    public function generate_private($public = '')
    {
        ...
        $key = mb_substr(md5($this->key . $public), 16 - $this->chars / 2, $this->chars);
    ...
    public function notvalid_msg()
    {
        // blank line for all languages

        if (1 == $this->current_try) {
            return '&nbsp;<br>&nbsp;';
        }

        // invalid try's: en

        if ('en' == $this->lang && $this->current_try > 2 && $this->refreshlink) {
            return 'No valid entry. Please try again:<br>Tipp: If you cannot identify the chars, you can generate a new image!';
        }

        if ('en' == $this->lang && $this->current_try >= 2) {
            return 'No valid entry. Please try again:<br>&nbsp;';
        }

        // invalid try's: de

        if ('de' == $this->lang && $this->current_try > 2 && $this->refreshlink) {
            return 'Die Eingabe war nicht korrekt.<br>Tipp: Wenn Du die Zeichen nicht erkennen kannst, generiere neue mit dem Link unten!';
        }

        if ('de' == $this->lang && $this->current_try >= 2) {
